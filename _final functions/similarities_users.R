...
#' @import coop
#' @export similarities_users
#' @name similarities_users
...
#' @examples
#' user_based <- similarities_users(book_ratings)
#'
...
similarities_users <- function(ratings_wide){
  coop::tcosine(ratings_wide)  # the tcosine function calculates cosine similarity between rows (users)
  diag(ratings_wide) <- 0      # set diagonal to zero to avoid returning similarity to self
  return(ratings_wide)
}
