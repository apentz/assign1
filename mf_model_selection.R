...
#' @description Train multiple models by varying parameters and compare RMSE on
#'   the training and out of sample (testing) set
#' @details The input dataset is a long matrix which contains User ID's, Item
#'   ID's and Ratings in the first 3 columns respectively. The output is a grid
#'   of parameters that were varied and the RMSE on the training and test set
#'   respectively.
#' @import dplyr
#' @import NNLM
...
#'
#' @param ratings_long The input dataset is a long matrix which contains User
#'   ID's, Item ID's and Ratings in the first 3 columns respectively.
#' @param k_latent Set method = "user" to return cosine similarities between
#'   rows (users) eg. readers who gave similar books similar ratings would have
...
mf_model_selection <- function(ratings_long, k_latent){
  ...
  # generate rmse for all values of k specified on training and test sets
...
k_rmse <- data.frame(k_latent = k_latent,
                     rmse_train = mf_model_selection(ratings_long)[1],
