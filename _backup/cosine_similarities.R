...
#' @description Calculates cosine similarities between either users or items eg.
#'   books or movies
...
#'   similarity scores between each User and Item combination.
...
#'   optimises computational performance. For more
...
#'   score of 1 indicates that users or items are similar to each other and a
#'   cosine similarity score of 0 indicates that users or items are differnt
#'   from each other with numbers between 0 and 1 depicting relative similarity..
#' @examples
...
cosine_similarities <- function(ratings_wide, method){
  ...
  if(method == "user"){
    ...
    user_similarities <- ratings_wide %>%
      ...
      diag() <- 0 %>%             # set diagonal to zero to avoid returning similarity to self
