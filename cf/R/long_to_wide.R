...
#'   collaborative filtering functions namely: user based, item based and matrix
#'   factorisation.
#' @import dplyr
#' @export long_to_wide
#' @name long_to_wide
...
#' @examples
#' ratings_wide_all <- long_to_wide(book_ratings, 0)
#  ratings_wide_test <- long_to_wide(users_test, 0)
#  ratings_wide_new <- long_to_wide(users_new, 0)
#'
#' @param ratings_long The input dataset contains 3 columns for User ID, Item
#' @param fill Missing values are populated either with zeros (fill = 0) or NAs
#'   (fill = NA)
