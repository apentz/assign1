#' @title Movie Ratings Data
#' @description Ratings (on a scale of 0 to 10) of 150 books from 10,000 users.
#' @details This is a modified version of the "Movie Lens" dataset.
#'   (\link{https://grouplens.org/datasets/movielens/}). The .RData file
#'   contains a small subset of 15 users and 20 movies and whether they have
#'   seen (rating=1) or not seen (rating=0) each movie. The full dataset has
#'   ratings on 40 000 movies by 260 000 users, some 24 million ratings in all.
#'   There are four csv files: `links.csv`, `movies.csv`, `ratings.csv`, and
#'   `tags.csv`.
#' @name viewed_movies
#' @docType data
#' @usage viewed_movies
#' @format tibble
#'
NULL   # to ensure this file is run when the package is loaded
